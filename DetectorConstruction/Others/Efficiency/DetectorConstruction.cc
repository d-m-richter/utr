...
//  const double filter_thickness = 0.5 * mm;           // <------ adjust filter thickness
 ...
 // G4Material *PLA = nist->FindOrBuildMaterial("G4_PLA");
  ...
  HPGe_GC2018_props.detector_radius = 0.5 * 60.6 * mm; // 0.5 * 60.6 mm with 0.2 mm uncertainty
  HPGe_GC2018_props.detector_length = 33.3 * mm; // 33.3 mm with 0.3 mm uncertainty 
  ...
  HPGe_GC2018_props.hole_radius = 0.5 * 9.5 * mm; // 9.5 * mm; 
  HPGe_GC2018_props.hole_depth = 16.0 * mm; // 16. * mm;
  HPGe_GC2018_props.hole_face_radius = 0. * mm; // {{hole_face_radius}} // 0. * mm
  HPGe_GC2018_props.dead_layer_side = 0.6 * mm; // 0.6 * mm
  HPGe_GC2018_props.dead_layer_top = 0.6 * mm; // 0.6 * mm
  ...
  HPGe_GC2018_props.mount_cup_under_window_thickness = 0.5 * mm; //0.5 * mm; // {{mount_cup_under_face_thickness}};
  ...
  HPGe_GC2018_props.end_cap_to_crystal_gap_front = 3.5 * mm; // 3.5 * mm with 0.5 mm uncertainty 
  HPGe_GC2018_props.end_cap_thickness = 1.5 * mm; // {{end_cap_thickness}} * mm; // 1.5 * mm Al with 0.1 mm uncertainty
  HPGe_GC2018_props.end_cap_window_thickness = 1.5 * mm; // 1.5 * mm Al with 0.1 mm uncertainty
 ...
 //     HPGe_GC2018_props.end_cap_to_crystal_gap_front; // Calculated from end cap length
  ...
  /***************** Edelstahlzylinder *********************************/
  ///*
  G4double Edelstahlzylinder_position = - 10 *mm;

  G4Tubs *Edelstahlzylinder_solid = new G4Tubs("Edelstahlzylinder_Solid", 0 *mm, 0.5 * 10.0 *mm, 0.5 * 20.0 *mm, 0. *deg, 360. *deg); // für die Simulation mit mehreren Petrischalen wurde 117.5 mm für den Durchmesser verwendet
  G4LogicalVolume *Edelstahlzylinder_logical = new G4LogicalVolume(Edelstahlzylinder_solid, stainlesssteel, "Edelstahlzylinder_Logical");
  G4PVPlacement *Edelstahlzylinder_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, Edelstahlzylinder_position), Edelstahlzylinder_logical, "Edelstahlzylinder", world_logical, false, 0);
  //*/
