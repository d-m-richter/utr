...
/*********************** Aluminumgehäuse besthet hier aus Stangen ****************************/
...
class GC_AluTop {
  ...
  GC_AluTop(G4LogicalVolume *world_Logical) {
    L = 402. * mm;
    M = 10. * mm;
    S = 368. * mm;
...
class GC_AluBottom_withHole {
  ...
  GC_AluBottom_withHole(G4LogicalVolume *world_Logical) {
    L = 5. * mm;
    M = 298. * mm;
    S = 298. * mm;
...
class GC_NormBrick {
  ...
  GC_NormBrick(G4LogicalVolume *world_Logical) {
    L = 200. * mm;
    M = 100. * mm;
    S = 50. * mm;
...
class GC_SlimNormBrick {
  private:
  G4LogicalVolume *World_Logical;

  G4LogicalVolume *SlimNormBrick_Logical;
  G4Box *SlimNormBrick_Solid;
  G4RotationMatrix *rot;

  G4double SlimNormBrick_medium;
  G4double SlimNormBrick_short;
  G4double SlimNormBrick_long;
  ...
  GC_SlimNormBrick(G4LogicalVolume *world_Logical) {
    L = 200. * mm;
    M = 100. * mm;
    S = 25. * mm;
    ...
    SlimNormBrick_Solid =
        new G4Box("SlimNormBrick_Solid", M / 2., S / 2., L / 2.);
    SlimNormBrick_Logical = new G4LogicalVolume(
        SlimNormBrick_Solid, Pb, "SlimNormBrick_Logical", 0, 0, 0);

    SlimNormBrick_Logical->SetVisAttributes(new G4VisAttributes(magenta));
  ...
  void Put(G4double x, G4double y, G4double z) {
    new G4PVPlacement(0, G4ThreeVector(x, y, z), SlimNormBrick_Logical,
                      "SlimNormBrick", World_Logical, false, 0);
  }
  ...
  void Put(G4double x, G4double y, G4double z, G4double angle_x,
    ...
    new G4PVPlacement(rot, G4ThreeVector(x, y, z), SlimNormBrick_Logical,
                      "SlimNormBrick", World_Logical, false, 0);
  }
};
