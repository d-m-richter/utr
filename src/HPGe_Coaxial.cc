...
//*/
  ...
  // Mount cup side
  G4double mount_cup_face_thickness = properties.mount_cup_upper_window_thickness + properties.mount_cup_under_window_thickness;
  ...
  G4double mount_cup_side_length = properties.mount_cup_length - mount_cup_face_thickness - properties.mount_cup_base_thickness; // properties.mount_cup_length - properties.mount_cup_thickness - properties.mount_cup_base_thickness;
///*
...
//  else if (mount_cup_face_thickness = properties.mount_cup_upper_window_thickness + properties.mount_cup_under_window_thickness) {
  ...
  mount_cup_upper_window_logical->SetVisAttributes(new G4VisAttributes(G4Color::Cyan()));
  ...
  mount_cup_under_window_logical->SetVisAttributes(new G4VisAttributes(G4Color::Cyan()));
...
//  }
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., -end_cap_side_length * 0.5 + properties.end_cap_to_crystal_gap_front + properties.mount_cup_upper_window_thickness + properties.mount_cup_under_window_thickness + mount_cup_side_length + 0.5 * properties.mount_cup_base_thickness), mount_cup_base_logical, detector_name + "_mount_cup_base", end_cap_vacuum_logical, 0, 0, false);
//*/
  /************* Cold finger *************/
...
*/
  ...
  G4double detector_radius = properties.detector_radius - properties.dead_layer_side;
  G4double detector_length = properties.detector_length - 2 * properties.dead_layer_top;
  G4double hole_radius = properties.hole_radius + properties.hole_dead_layer;
  G4double hole_depth = properties.hole_depth + properties.hole_dead_layer - properties.dead_layer_top;
  //G4double hole_face_radius = properties.hole_face_radius + properties.hole_dead_layer;
  G4double detector_face_radius = properties.detector_face_radius - properties.dead_layer_top;
  G4double detector_back_radius = 0.;//properties.detector_back_radius - properties.dead_layer_top;

  G4double heigth_of_curvature = 0.;
  G4double heigth_of_curvature_inner = 0.;
  G4double hole_face_radius = 0.;

  if (properties.hole_face_radius != 0.) {
    hole_face_radius = properties.hole_face_radius + properties.hole_dead_layer;
    heigth_of_curvature = hole_face_radius - sqrt(pow(hole_face_radius, 2) - pow(hole_radius, 2)); // if hole_face_radius =/ hole_radius
    heigth_of_curvature_inner = properties.hole_face_radius - sqrt(pow(properties.hole_face_radius, 2) - pow(properties.hole_radius, 2));
  } 
  ...
  for (int i = 0; i < nsteps; i++) {
    ...
    if (z >= properties.detector_face_radius && z <= properties.detector_length - properties.detector_back_radius) {
      rOuterTemp[i] = properties.detector_radius;
    } else if (z >= 0. && z <= properties.detector_face_radius) {
    ...
    } else if (z >= properties.detector_length - properties.detector_back_radius && z <= properties.detector_length) {
      rOuterTemp[i] = properties.detector_back_radius * sqrt(1. - pow((z - (properties.detector_length - properties.detector_back_radius)) / properties.detector_back_radius, 2)) + properties.detector_radius - properties.detector_back_radius;
    } else {
      rOuterTemp[i] = 0. * mm;
    }
  }

  G4int nsteps_optimized = opt->Optimize(zPlaneTemp, rInnerTemp, rOuterTemp, zPlane, rInner, rOuter, nsteps, detector_name + "_dead_layer_solid");

  G4Polycone *dead_layer_solid = new G4Polycone("dead_layer_solid", 0. * deg, 360. * deg, nsteps_optimized, zPlane, rInner, rOuter);
  //G4SubtractionSolid * layer_solid = new G4SubtractionSolid("layer_solid", whole_crystal_solid, crystal_solid, 0, G4ThreeVector(0, 0, properties.dead_layer_top));
  //G4SubtractionSolid * dead_layer_solid = new G4SubtractionSolid("dead_layer_solid", layer_solid, ring_solid, 0, G4ThreeVector(0, 0, properties.detector_length - 2./2.));

  G4LogicalVolume *dead_layer_logical = new G4LogicalVolume(dead_layer_solid, nist->FindOrBuildMaterial(properties.dead_layer_material), detector_name + "_dead_layer", 0, 0, 0);
  dead_layer_logical->SetVisAttributes(new G4VisAttributes(G4Color::Blue()));
  new G4PVPlacement(0, G4ThreeVector(0., 0., -end_cap_side_length * 0.5 + properties.end_cap_to_crystal_gap_front + properties.mount_cup_upper_window_thickness + properties.mount_cup_under_window_thickness), dead_layer_logical, detector_name + "_dead_layer", end_cap_vacuum_logical, 0, 0, false); 

  // inner crystal

  for (int i = 0; i < nsteps; i++) {
    z = (1. - (double)i / (nsteps - 1)) * detector_length;

    zPlaneTemp[i] = z;

    if (z >= detector_length - hole_depth) {
      rInnerTemp[i] = hole_radius;
    } else {
      rInnerTemp[i] = 0.;
    }

    if (z >= detector_face_radius && z <= detector_length - detector_back_radius) {
      rOuterTemp[i] = detector_radius;
    } else if (z >= 0. && z <= detector_face_radius) {
      rOuterTemp[i] = detector_face_radius * sqrt(1. - pow((z - detector_face_radius) / detector_face_radius, 2)) + (detector_radius - detector_face_radius);
    } else if (z >= detector_length - detector_back_radius && z <= detector_length) {
      rOuterTemp[i] = detector_back_radius * sqrt(1. - pow((z - (detector_length - detector_back_radius)) / detector_back_radius, 2)) + detector_radius - detector_back_radius;
    } else {
      rOuterTemp[i] = 0. * mm;
    }
  }

  G4int msteps_optimized = opt->Optimize(zPlaneTemp, rInnerTemp, rOuterTemp, zPlane, rInner, rOuter, nsteps, detector_name + "_crystal_solid");

  G4Tubs *ring_solid = new G4Tubs("ring_solid", 20./2. *mm, 28./2. *mm, 2./2. *mm, 0. *deg,  360. *deg);

  G4Polycone *crystals_solid = new G4Polycone("crystals_solid", 0. * deg, 360. * deg, msteps_optimized, zPlane, rInner, rOuter);
  G4SubtractionSolid * crystal_solid = new G4SubtractionSolid("crystal_solid", crystals_solid, ring_solid, 0, G4ThreeVector(0, 0, detector_length - 2./2. + properties.dead_layer_top));
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., properties.dead_layer_top), crystal_logical, detector_name + "_crystal", dead_layer_logical, 0, 0, false);
  

  //G4Tubs *ring_solid = new G4Tubs("ring_solid", 20./2. *mm, 28./2. *mm, 2./2. *mm, 0. *deg,  360. *deg);  
  G4LogicalVolume *ring_logical = new G4LogicalVolume(ring_solid, nist->FindOrBuildMaterial("G4_Galactic"), detector_name + "_vacuum_ring", 0, 0, 0);
  ring_logical->SetVisAttributes(new G4VisAttributes(G4Color::White()));
  new G4PVPlacement(0, G4ThreeVector(0., 0., properties.detector_length - 2./2.), ring_logical, detector_name + "_vacuum_ring", dead_layer_logical, 0, 0, false); 
...
/*  
  ...
  //}
    ...
    // rInnerTemp[i] = 0. * mm;
    //if (z >= detector_length - hole_depth) {
    //  if (z >= detector_length - hole_depth + heigth_of_curvature) {
    //  rInnerTemp[i] = hole_radius;
    //  } else {
    //    rInnerTemp[i] = hole_radius * sqrt(1. - pow((z - (detector_length - hole_depth + heigth_of_curvature)) / hole_radius, 2));
    //  }
    //} else {
    //  rInnerTemp[i] = 0.;
    //}
  ...
  G4LogicalVolume *crystal_logical = new G4LogicalVolume(crystal_solid, nist->FindOrBuildMaterial("G4_Ge"), detector_name, 0, 0, 0);
  crystal_logical->SetVisAttributes(new G4VisAttributes(G4Color::Green()));
  new G4PVPlacement(0, G4ThreeVector(0., 0., -end_cap_side_length * 0.5 + properties.end_cap_to_crystal_gap_front + mount_cup_face_thickness + properties.dead_layer_top), crystal_logical, detector_name + "_crystal", end_cap_vacuum_logical, 0, 0, false);
  */
  /******************** Inner dead layer *********************************/
/* 
  const G4int ksteps = 10000;
  ...
  for (int i = 0; i < ksteps; i++) {
    z = (1. - (double)i / (ksteps - 1)) * hole_depth;

    zPlaneTempk[i] = z;

    //if (z >= properties.hole_dead_layer) {
    //  if(z >= heigth_of_curvature_inner){
    //    rInnerTempk[i] = properties.hole_radius;
    //  } else {
    //    rInnerTempk[i] = (properties.hole_radius) * sqrt(1. - pow((z - heigth_of_curvature_inner) / (properties.hole_radius), 2));
    //  }    
    //} else {
    //  rInnerTempk[i] = 0;
    //}
    ...
    //  rOuterTempk[i] = hole_radius;
    //} else {
    //  rOuterTempk[i] = (hole_radius) * sqrt(1. - pow((z - (heigth_of_curvature)) / (hole_radius), 2));
    //}
    if (z >= 0.) {
      rOuterTempk[i] = hole_radius;
    } else {
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., -end_cap_side_length * 0.5 + properties.end_cap_to_crystal_gap_front + mount_cup_face_thickness + properties.detector_length - properties.hole_depth - properties.hole_dead_layer + 0.000001), hole_dead_layer_logical, detector_name + "_hole_dead_layer", end_cap_vacuum_logical, 0, 0, false);
*/  
...
/*  
  ...
  for (int i = 0; i < msteps; i++) {
    z = (1. - (double)i / (msteps - 1)) * properties.detector_length; //(properties.detector_length - properties.detector_back_radius);
    ...
    if (z >= properties.dead_layer_top) {
      if (z >= properties.detector_face_radius) {
        rInnerTempm[i] = detector_radius;
      } else {
        rInnerTempm[i] = (detector_face_radius) * sqrt(1. - pow((z - (detector_face_radius)) / (detector_face_radius), 2)) + (detector_radius - detector_face_radius);
      }
    } else {
      rInnerTempm[i] = 0.;
    }
    ...
    if (z >= properties.detector_face_radius) {
      rOuterTempm[i] = properties.detector_radius;
    } else if (z >= 0. && z <= properties.detector_face_radius) {
      rOuterTempm[i] = (properties.detector_face_radius) * sqrt(1. - pow((z - (properties.detector_face_radius)) / (properties.detector_face_radius), 2)) + (properties.detector_radius - properties.detector_face_radius);
    } else {
      rOuterTempm[i] = 0. * mm;
    }
  }
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., -end_cap_side_length * 0.5 + properties.end_cap_to_crystal_gap_front + mount_cup_face_thickness), outer_dead_layer_logical, detector_name + "_outer_dead_layer", end_cap_vacuum_logical, 0, 0, false);
*/  
  ...
  //G4LogicalVolume *dead_layer_top_logical = new G4LogicalVolume(dead_layer_top_solid, nist->FindOrBuildMaterial(properties.dead_layer_material), detector_name + "_dead_layer_top_logical", 0, 0, 0);
  //dead_layer_top_logical->SetVisAttributes(new G4VisAttributes(G4Color(1., 0.5, 1.)));
