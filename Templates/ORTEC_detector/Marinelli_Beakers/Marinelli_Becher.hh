...
class Marinelli_Holder {
  private:
  G4LogicalVolume *World_Logical;
  G4Polycone *Marinelli_Holder_Solid;
  G4LogicalVolume *Marinelli_Holder_Logical;
  G4RotationMatrix *rot;

  public:
  G4double inner_radius; // inner radius
  G4double outer_outer_radius; // outer radius
  G4double inner_outer_radius;
  G4double inner_lenght; // 
  G4double outer_under_lenght; // 
  G4double outer_upper_lenght; // 

  // for all geometries
  G4double sphi;
  G4double dphi;

  Marinelli_Holder(G4LogicalVolume *world_Logical) {
    inner_radius = 0.5 * 82. * mm;
    outer_outer_radius = 0.5 * 136. * mm;
    inner_outer_radius = 0.5 * 96. *mm;
    inner_lenght = 42. * mm;
    outer_under_lenght = 31.2 * mm;
    outer_upper_lenght = 10.8 * mm;
    ...
    G4int numZPlanes = 3; 
    G4double rInner = {inner_radius, inner_radius, inner_radius}; 
    G4double rOuter = {outer_outer_radius, inner_outer_radius, inner_outer_radius}; 
    G4double zPlane = {0, outer_under_lenght, outer_upper_lenght};

    Marinelli_Holder_Solid = new G4Polycone("Marinelli_Holder_Solid", phiStart, phiTotal, numZPlanes, zPlane, rInner, rOuter);

    // Deckel
    Marinelli_Holder_Logical = new G4LogicalVolume(Marinelli_Holder_Solid, polyethylene, "Marinelli_Holder_Logical", 0, 0, 0);

    Marinelli_Holder_Logical->SetVisAttributes(new G4VisAttributes(magenta));
  ...
  // use Marinelli *marinelli = new Marinelli(world_logical) and marinelli->Put(0., 0., 90.)
  void Put(G4double x, G4double y, G4double z) {
    new G4PVPlacement(0, G4ThreeVector(x, y, z), Marinelli_Holder_Logical,
                      "Marinelli_Holder", World_Logical, false, 0);
  }
  ...
  void Put(G4double x, G4double y, G4double z, G4double angle_x,
    ...
    new G4PVPlacement(rot, G4ThreeVector(x, y, z), Marinelli_Holder_Logical,
                      "Marinelli_Holder", World_Logical, false, 0);
  }
};
