 ...
 * 
 * This simulation setup is used for clearance purpose of the steel bottels which were used in QCLAM experiments.
 * They were filled with argon in which Co-60 was activated.
 */
...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  HPGe_GEM20P470_props.hole_radius = 0.5 * {{hole_diameter}} - HPGe_GEM20P470_props.hole_dead_layer * mm; // 9.6 * mm; 
  HPGe_GEM20P470_props.hole_depth = {{hole_depth}} - HPGe_GEM20P470_props.hole_dead_layer * mm; // 51.5 * mm;
  ...
  HPGe_GEM20P470_props.mount_cup_upper_window_material = "G4_Mylar"; // 0.03 * mm Mylar
 ...
 //     HPGe_GEM20P470_props.end_cap_to_crystal_gap_front; // Calculated from end cap length
  ...
  const double world_x =  2. *m; //HPGe_GEM20P470_props.dewar_outer_radius;
  const double world_y =  2. *m; //HPGe_GEM20P470_props.dewar_outer_radius;
  const double world_z =  2. *m; //2 * HPGe_GEM20P470_props.dewar_length + distance_source_detector;
  ...
  /***************** 4 Edelgasflschen um den Detektor herum gestellt ************************************************/

  G4double Edelgasflasche_position = 267 * mm;

  G4Tubs *Edelgasflasche_solid = new G4Tubs("Edelgasflasche_Solid", 0 *mm, {{Edelgasflasche_radius}} *mm, 0.5 * 1420 *mm, 0. *deg, 360. *deg);
  G4LogicalVolume *Edelgasflasche_logical = new G4LogicalVolume(Edelgasflasche_solid, Stainless_Steel, "Edelgasflasche_Logical");
  
  G4Colour green(0., 1., 0.);
  Edelgasflasche_logical->SetVisAttributes(new G4VisAttributes(green));

  G4PVPlacement *Edelgasflasche1_phys = new G4PVPlacement(0, G4ThreeVector(200 * mm , 0, Edelgasflasche_position), Edelgasflasche_logical, "Edelgasflasche1", world_logical, false, 0);
  G4PVPlacement *Edelgasflasche2_phys = new G4PVPlacement(0, G4ThreeVector(-200 * mm, 0, Edelgasflasche_position), Edelgasflasche_logical, "Edelgasflasche2", world_logical, false, 0);
  G4PVPlacement *Edelgasflasche3_phys = new G4PVPlacement(0, G4ThreeVector(0, 200 * mm, Edelgasflasche_position), Edelgasflasche_logical, "Edelgasflasche3", world_logical, false, 0);
  G4PVPlacement *Edelgasflasche4_phys = new G4PVPlacement(0, G4ThreeVector(0, -200 * mm, Edelgasflasche_position), Edelgasflasche_logical, "Edelgasflasche4", world_logical, false, 0);
...
/*
  ...
  alufront_out1->Put(160., 0., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 0.);
  // rechts
  AluFront_out *alufront_out2 = new AluFront_out(world_logical);
  alufront_out2->Put(-160., 0., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 0.);
  // oben
  AluSide_out *aluside_out1 = new AluSide_out(world_logical);
  aluside_out1->Put(0., 160., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 90. *deg);
  // unten
  AluSide_out *aluside_out2 = new AluSide_out(world_logical);
  aluside_out2->Put(0., -160., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 90. *deg);

  // Aluplatten innen

  // links
  AluFront_in *alufront_in1 = new AluFront_in(world_logical);
  alufront_in1->Put(93.75, 0., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 0.);
  // rechts
  AluFront_in *alufront_in2 = new AluFront_in(world_logical);
  alufront_in2->Put(-93.75, 0., distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 0.);
  // oben
  AluSide_in *aluside_in1 = new AluSide_in(world_logical);
  aluside_in1->Put(0., 93.75, distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 90. *deg);
  // unten
  AluSide_in *aluside_in2 = new AluSide_in(world_logical);
  aluside_in2->Put(0., -93.75, distance_source_detector + 135. - (135 - 125) - 300./2., 0., 0., 90. *deg);
  ...
  alubottom_withhole->Put(0., 0., distance_source_detector + 127. - 5./2., 0., 0., 0.);

  // Bleiblöcke 1. Reihe

  // Reihe oben
  NormBrick *normbrick_1o = new NormBrick(world_logical);
  normbrick_1o->Put(51.25, 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_1o = new ShortNormBrick(world_logical);
  shortnormbrick_1o->Put(-100., 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 90. *deg, 90. *deg);
  
  // Reihe links
  NormBrick *normbrick_1l = new NormBrick(world_logical);
  normbrick_1l->Put(122.5, -0.125, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 0. *deg, 90. *deg);
  
  // Reihe unten
  NormBrick *normbrick_1u = new NormBrick(world_logical);
  normbrick_1u->Put(-51.25, -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_1u = new ShortNormBrick(world_logical);
  shortnormbrick_1u->Put(100., -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 90. *deg, 90. *deg);
  
  // Reihe rechts
  NormBrick *normbrick_1r = new NormBrick(world_logical);
  normbrick_1r->Put(-122.5, 0.625, distance_source_detector + 135. - (135 - 125) + 5. - 100./2., 90. *deg, 0. *deg, 90. *deg);

  // Bleiblöcke 2. Reihe

  // Reihe oben
  NormBrick *normbrick_2o = new NormBrick(world_logical);
  normbrick_2o->Put(51.25, 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_2o = new ShortNormBrick(world_logical);
  shortnormbrick_2o->Put(-100., 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);
  
  // Reihe links
  NormBrick *normbrick_2l = new NormBrick(world_logical);
  normbrick_2l->Put(122.5, -0.125, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 0. *deg, 90. *deg);

  // Reihe unten
  NormBrick *normbrick_2u = new NormBrick(world_logical);
  normbrick_2u->Put(-51.25, -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_2u = new ShortNormBrick(world_logical);
  shortnormbrick_2u->Put(100., -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  // Reihe rechts
  NormBrick *normbrick_2r = new NormBrick(world_logical);
  normbrick_2r->Put(-122.5, 0.625, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100./2., 90. *deg, 0. *deg, 90. *deg);

  // Bleiblöcke 3. Reihe

  // Reihe oben
  NormBrick *normbrick_3o = new NormBrick(world_logical);
  normbrick_3o->Put(51.25, 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_3o = new ShortNormBrick(world_logical);
  shortnormbrick_3o->Put(-100., 122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);
  
  // Reihe links
  NormBrick *normbrick_3l = new NormBrick(world_logical);
  normbrick_3l->Put(122.5, -0.125, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 0. *deg, 90. *deg);
  
  // Reihe unten
  NormBrick *normbrick_3u = new NormBrick(world_logical);
  normbrick_3u->Put(-51.25, -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);

  ShortNormBrick *shortnormbrick_3u = new ShortNormBrick(world_logical);
  shortnormbrick_3u->Put(100., -122.5, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 90. *deg, 90. *deg);
  
  // Reihe rechts
  NormBrick *normbrick_3r = new NormBrick(world_logical);
  normbrick_3r->Put(-122.5, 0.625, distance_source_detector + 135. - (135 - 125) + 5. - 100. - 100. - 100./2., 90. *deg, 0. *deg, 90. *deg);
*/
