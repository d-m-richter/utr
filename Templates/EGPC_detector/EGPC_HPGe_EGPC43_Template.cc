 ...
 *
 * The detector is a p-type coaxial HPGe detector with a relative efficiency of 43.3 % at 1332 keV from Eurisys Mesure.
 */
...
//  const double filter_thickness = 0.5 * mm;           // <------ adjust filter thickness
 ...
 // G4Material *PLA = nist->FindOrBuildMaterial("G4_PLA");
  ...
  HPGe_Coaxial_Properties HPGe_radiationprotection_props;
  // Volume of Ge Crystal = 166 * cm3
  // Dead layer of Ge Crystal = 1mmGe
  HPGe_radiationprotection_props.detector_radius = 0.5 * {{cystal_diameter}} * mm; // 64 mm in EurisysMesure data sheet
  HPGe_radiationprotection_props.detector_length = {{crystal_length}} * mm; // 53 mm in EurisysMesure data sheet
  HPGe_radiationprotection_props.detector_face_radius = 0. * mm;
  HPGe_radiationprotection_props.hole_radius = 0.5 * {{hole_diameter}} * mm; // 11.35 * mm; 
  HPGe_radiationprotection_props.hole_depth = {{hole_depth}} * mm; // 44.5 * mm;
  HPGe_radiationprotection_props.dead_layer_side = {{dead_layer_side}} * mm; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.dead_layer_top = {{dead_layer_top}} * mm; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.mount_cup_thickness = 0.8 * mm;
  HPGe_radiationprotection_props.mount_cup_base_thickness = 3. * mm;
  HPGe_radiationprotection_props.mount_cup_material = "G4_Al";
  HPGe_radiationprotection_props.end_cap_outer_radius = 0.5 * 80. * mm; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.end_cap_to_crystal_gap_front = {{distance_detector_end_cap}} * mm; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.end_cap_thickness = {{end_cap_thickness}} * mm; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.end_cap_window_thickness = {{end_cap_face_thickness}} * mm;
  HPGe_radiationprotection_props.end_cap_length = {{end_cap_stand_out}} + 3 *mm; // gemessen 132. * mm; // 135 mm in EurisysMesure data sheet
  HPGe_radiationprotection_props.end_cap_material = "G4_Al"; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.end_cap_window_material = "G4_Al"; // in EurisysMesure data sheet
  HPGe_radiationprotection_props.cold_finger_radius = 0.5 * 4. * mm; // Suggestion by B. Fallin, Duke University. Not in ORTEC data sheet
  HPGe_radiationprotection_props.cold_finger_material = "G4_Cu"; // Estimated
  HPGe_radiationprotection_props.connection_length = 1.5 * inch;
  HPGe_radiationprotection_props.connection_radius = 0.75 * inch; // Estimated
  HPGe_radiationprotection_props.dewar_offset = 0. * inch;
  HPGe_radiationprotection_props.connection_material = "G4_Al"; // Estimated
  HPGe_radiationprotection_props.dewar_length = 12.5 * inch;
  HPGe_radiationprotection_props.dewar_outer_radius = 4.5 * inch;
  HPGe_radiationprotection_props.dewar_wall_thickness = 5. * mm; // Estimated
  HPGe_radiationprotection_props.dewar_material = "G4_Al"; // Estimated
  HPGe_radiationprotection_props.hole_face_radius = HPGe_radiationprotection_props.hole_radius; // Estimated to be the same as hole radius
  HPGe_radiationprotection_props.cold_finger_penetration_depth = HPGe_radiationprotection_props.hole_depth - 5. * mm; // Estimated
  HPGe_radiationprotection_props.end_cap_to_crystal_gap_side =
      HPGe_radiationprotection_props.end_cap_outer_radius -
      HPGe_radiationprotection_props.end_cap_thickness -
      HPGe_radiationprotection_props.mount_cup_thickness -
      HPGe_radiationprotection_props.detector_radius -
      HPGe_radiationprotection_props.dead_layer_side; // Calculated from outer radius and other given dimensions
  HPGe_radiationprotection_props.mount_cup_length =
      HPGe_radiationprotection_props.end_cap_length -
      HPGe_radiationprotection_props.end_cap_thickness -
      HPGe_radiationprotection_props.end_cap_to_crystal_gap_front; // Calculated from end cap length


  /***************** World Volume *****************/

  const double world_x =  1. *m; //HPGe_radiationprotection_props.dewar_outer_radius;
  const double world_y =  1. *m; //HPGe_radiationprotection_props.dewar_outer_radius;
  const double world_z =  1. *m; //2 * HPGe_radiationprotection_props.dewar_length + distance_source_detector;
  ...
  HPGe_Coaxial *HPGe_radiationprotection= new HPGe_Coaxial(world_logical, "Detector_logical");
  (*HPGe_radiationprotection).setProperties(HPGe_radiationprotection_props);
  //HPGe_radiationprotection.useDewar();
  //(*HPGe_radiationprotection).Add_Filter(filter_material_name, filter_thickness, HPGe_radiationprotection_props.detector_radius + 2 *HPGe_radiationprotection_props.dewar_wall_thickness);
  (*HPGe_radiationprotection).Construct(G4ThreeVector(0., 0., 0), 0, twopi, distance_source_detector, 0);
