...
//  const double filter_thickness = 0.5 * mm;           // <------ adjust filter thickness
 ...
 //     HPGe_GC2018_props.end_cap_to_crystal_gap_front; // Calculated from end cap length
  ...
  G4double folie_position = -82.72 - 0.8/2. *mm ; // - 15.35 6.5
  ...
  /***************** Folienhalter ************************************************/
  ///*
  G4double folienhalterung_position = -82.72 + 6.5 *mm; 
  ...
  /***************** Hohlzylinder *********************************/
  ///*
  G4double hohlzylinder_position = -82.72 + 6.5 + 170./2. *mm;

  G4Tubs *hohlzylinder_solid = new G4Tubs("Hohlzylinder_Solid", 0.5 * 81.0 *mm, 0.5 * 102.0 *mm, 0.5 * 170.0 *mm, 0. *deg, 360. *deg); // für die Simulation mit mehreren Petrischalen wurde 117.5 mm für den Durchmesser verwendet
