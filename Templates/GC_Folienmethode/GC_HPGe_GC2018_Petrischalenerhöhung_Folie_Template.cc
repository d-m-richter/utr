...
//  const double filter_thickness = 0.5 * mm;           // <------ adjust filter thickness
 ...
 //     HPGe_GC2018_props.end_cap_to_crystal_gap_front; // Calculated from end cap length
  ...
  /***************** Folienstandard ************************************************/
  ///*
  // The under side of the foil or petri dish is placed at 12.72 mm distance to the detector 
  G4double folie_position = -12.72 - 15.35 - 0.8/2. *mm ; // - 15.35 6.5
  ...
  G4PVPlacement *folie_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, {{position}}), folie_logical, "Folie", world_logical, false, 0);
  //*/
  /************************** Plastikfolie, in welche Wasser Petrischale eingeschweißt wurde **************/
  /*
  G4double schweisfolie_oben = -12.72 - 15.7 - 0.1 - 0.1/2. *mm ;
  G4double schweisfolie_unten = -12.72 - 0.1/2. *mm ;

  G4Tubs *schweisfolie_solid = new G4Tubs("Schweisfolie_Solid", 0 *mm, 0.5 * 100.0 *mm, 0.5 * 0.1 *mm, 0. *deg, 360. *deg);
  G4LogicalVolume *schweisfolie_logical = new G4LogicalVolume(schweisfolie_solid, polyethylene, "Schweisfolie_Logical");

  G4PVPlacement *schweisfolie_oben_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, schweisfolie_oben), schweisfolie_logical, "Schweisfolie_oben", world_logical, false, 0);
  G4PVPlacement *schweisfolie_unten_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, schweisfolie_unten), schweisfolie_logical, "Schweisfolie_unten", world_logical, false, 0);
  */
  /***************** Folienhalter ************************************************/
  ///*
  G4double folienhalterung_position = -12.72 + 6.5 *mm; 

  Folienhalterung *folienhalterung = new Folienhalterung(world_logical);
  folienhalterung->Put(0, 0, folienhalterung_position, 180 *deg, 0., 0.);
  //*/
  /***************** Zylinderhalterung ************************************************/
  ///*
  G4double zylinderhalterung_position = -12.72 - 2. *mm; 

  Zylinderhalterung *zylinderhalterung = new Zylinderhalterung(world_logical);
  zylinderhalterung->Put(0, 0, zylinderhalterung_position, 180 *deg, 0., 0.);
  //*/
  /***************** Petrischale außen ************************************************/
  ///*
  G4double petrischale_position = -12.72 *mm; 

  Petrischale *petrischale = new Petrischale(world_logical);
  petrischale->Put(0, 0, petrischale_position, 180 *deg, 0., 0.);
  //*/
  /***************** Petrischale innen ************************************************/
  ///*
  G4double fuelvolumen_position = -12.72 - 0.6 *mm;
  
  //G4double density = {{density}} * g/mm3; //(34.2 / 71125.658) * g/mm3;
  //G4Material* material = nist->FindOrBuildMaterial({{material}});
  //G4Material* material_new = nist->BuildMaterialWithNewDensity("G4_MATERIAL_NEW",{{material}},density);

  //G4Colour magenta(1., 0., 1.);

  Fuelvolumen *fuelvolumen = new Fuelvolumen(world_logical);
  //fuelvolumen->SetVisAttributes(new G4VisAttributes(magenta));
  //fuelvolumen->SetMaterial(material_new);
  fuelvolumen->Put(0, 0, fuelvolumen_position, 180 *deg, 0., 0.);
  //*/
  /***************** Hohlzylinder *********************************/
  ///*
  G4double hohlzylinder_position = -12.72 + 6.5 + 100./2. *mm;

  G4Tubs *hohlzylinder_solid = new G4Tubs("Hohlzylinder_Solid", 0.5 * 81.0 *mm, 0.5 * 102.0 *mm, 0.5 * 100.0 *mm, 0. *deg, 360. *deg); // für die Simulation mit mehreren Petrischalen wurde 117.5 mm für den Durchmesser verwendet
  G4LogicalVolume *hohlzylinder_logical = new G4LogicalVolume(hohlzylinder_solid, polyethylene, "Hohlzylinder_Logical");
  G4PVPlacement *hohlzylinder_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, hohlzylinder_position), hohlzylinder_logical, "Hohlzylinder", world_logical, false, 0);
  //*/
  /***************** Bleiburg *********************************/
