...
//  const double filter_thickness = 0.5 * mm;           // <------ adjust filter thickness
 ...
 //     HPGe_GC2018_props.end_cap_to_crystal_gap_front; // Calculated from end cap length
  ...
  /***************** Folienstandard ************************************************/
///*
  G4double folie_position = 93.875 - (30.0 + 40.0 + 100.0) - 6.5 -(0.5 * 0.8); // - 15.35 6.5
  ...
  //G4PVPlacement *folie_phys = new G4PVPlacement(0, G4ThreeVector({{x_variation}}, {{y_variation}}, {{z_variation}}), folie_logical, "Folie", world_logical, false, 0);
  G4PVPlacement *folie_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, {{z_variation}}), folie_logical, "Folie", world_logical, false, 0);
//*/

  /***************** Folienhalter ************************************************/

  ///*
  G4double folienhalterung_position = {{z_variation}} + 0.4 + 6.5 *mm; 
  ...
  //*/
  /*
  G4Tubs *oben = new G4Tubs("Oben", 50.5 *mm, 55.5 *mm, 0.5 * 2.0 *mm, 0. *deg, 360. *deg);
  G4Tubs *mitte = new G4Tubs("Mitte", 33.0 *mm, 55.5 *mm, 0.5 * 3.5 *mm, 0. *deg, 360. *deg);
  G4Tubs *unten = new G4Tubs("Unten", 41.0 *mm, 51.0 *mm, 0.5 * 3.0 *mm, 0. *deg, 360. *deg);

  G4UnionSolid *oben_mitte = new G4UnionSolid("Oben_Mitte", oben, mitte, 0, G4ThreeVector(0, 0, (2./2. + 3.5/2.)));
  G4UnionSolid *folienhalter_solid = new G4UnionSolid("Folienhalter_Solid", oben_mitte, unten, 0, G4ThreeVector(0, 0, (5.5/2. + 3./2.)));
  
  G4LogicalVolume *folienhalter_logical = new G4LogicalVolume(folienhalter_solid, polyethylene, "Folienhalter_Logical");
  G4Colour blue(0., 0., 1.);
  folienhalter_logical->SetVisAttributes(new G4VisAttributes(blue));
  G4PVPlacement *folienhalter_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, {{z_variation}}), folienhalter_logical, "Folienhalter", world_logical, false, 0);
  */
...
/*
  ...
  quellenhalter->Put({{horizontal_shift_x}}, {{horizontal_shift_y}}, quellenhalter_position, 180. *deg, 0., 0.);
*/
  /***************** Hohlzylinder *********************************/
/*
...
///*
  ...
  G4PVPlacement *hohlzylinder_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, {{z_variation}} - 1.6 + 8.5 + 100./2.), hohlzylinder_logical, "Hohlzylinder", world_logical, false, 0);
//*/
...
/*
  ...
  G4PVPlacement *plastikzylinder_phys = new G4PVPlacement(0, G4ThreeVector({{horizontal_shift_x}}, {{horizontal_shift_y}}, plastikzylinder_position), plastikzylinder_logical, "Plastikzylinder", world_logical, false, 0);
*/
  /***************** Bleiburg *********************************/
